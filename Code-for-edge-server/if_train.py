...
ATK_TR_CSV = os.path.join(BASE_DIR, "preprocessed", "rw_attacks_train.csv")
ATK_TE_CSV = os.path.join(BASE_DIR, "preprocessed", "rw_attacks_test.csv")
...
def load(path, required=True):
    if not os.path.exists(path):
        if required: sys.exit(f"Missing: {path}")
        return None, None
    df = pd.read_csv(path)
    if df.empty:
        return None, None
    ...
    if missing:
        sys.exit(f"{os.path.basename(path)} missing columns: {missing}\n"
                 f"Re-run preprocess scripts after updating core.py")
...
print("Loading data...")
X_train, _        = load(TRAIN_CSV)
X_val,   _        = load(VAL_CSV)
X_test,  _        = load(TEST_CSV)
X_atk_tr, y_atk_tr = load(ATK_TR_CSV, required=False)
X_atk_te, y_atk_te = load(ATK_TE_CSV, required=False)

for name, X in [("train",X_train),("val",X_val),("test",X_test),
                ("attack_train",X_atk_tr),("attack_test",X_atk_te)]:
    print(f"  {name:15s}: {len(X) if X is not None else 'not found'} windows")
...
s_val   = score(X_val)
s_test  = score(X_test)
s_atk_tr = score(X_atk_tr) if X_atk_tr is not None else None
s_atk_te = score(X_atk_te) if X_atk_te is not None else None

for name, s in [("train",s_train),("val",s_val),("test",s_test)]:
    print(f"  {name:8s}: mean={s.mean():.4f}  flagged={( s>threshold).mean():.1%}")
...
# Chart 2: normal vs attack (train attack data)
if s_atk_tr is not None:
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.hist(s_train,  bins=40, alpha=0.55, color=BLUE,   density=True,
            label=f"Normal  n={len(s_train)}")
    ax.hist(s_atk_tr, bins=40, alpha=0.55, color=ORANGE, density=True,
            label=f"Attack  n={len(s_atk_tr)}")
...
# Chart 3: per-class detection rate (bar chart)
if X_atk_tr is not None and y_atk_tr is not None:
    df_atk = pd.read_csv(ATK_TR_CSV)
    fig, ax = plt.subplots(figsize=(8, 4.5))
    labels_order = sorted(df_atk["label"].unique())
    rates  = []
    colors = [RED, ORANGE, "#7F77DD"]
    for lbl in labels_order:
        mask = df_atk["label"] == lbl
        s    = score(df_atk.loc[mask, FEATURES].values)
        rates.append((s > threshold).mean() * 100)

    bars = ax.bar(labels_order, rates, color=colors[:len(labels_order)],
    ...
    ax.set_ylim(0, 115)
    ax.set_title("IF — per-class detection rate (attack train set)")
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, "if_per_class_detection.png"), dpi=150, bbox_inches="tight")
    plt.close()
    print("\nPer-class detection (attack train):")
    for lbl, rate in zip(labels_order, rates):
        print(f"  {lbl:15s}: {rate:.1f}%")


# Chart 4: ROC curve
if X_atk_te is not None:
    y_true   = np.concatenate([np.zeros(len(s_test)), np.ones(len(s_atk_te))])
    y_scores = np.concatenate([s_test, s_atk_te])
    ...
    ax.set_title("IF — ROC curve (real-world test)")
    ax.legend()
    plt.tight_layout()
...
# Chart 5: confusion matrix using real-world test data
if X_atk_te is not None:
    n_atk = len(s_atk_te)
    y_true_cm = np.concatenate([np.zeros(len(s_test)), np.ones(n_atk)])
    y_pred_cm = np.concatenate([
        (s_test    > threshold).astype(int),
        (s_atk_te  > threshold).astype(int),
    ])
...
axes[0].set_xlabel("Contamination (%)"); axes[0].set_ylabel("Val windows flagged (%)")
axes[0].set_title("Flagged rate on val set"); axes[0].legend()
...
axes[1].axvline(CONTAMINATION*100, color=ORANGE, linestyle="--",
                linewidth=1.2, label=f"Chosen: {CONTAMINATION*100:.0f}%")
axes[1].set_xlabel("Contamination (%)"); axes[1].set_ylabel("Decision threshold")
axes[1].set_title("Threshold vs contamination"); axes[1].legend()
plt.tight_layout()
...
for j in range(X_val.shape[1]):
    Xp = X_val.copy(); Xp[:,j] = rng.permutation(Xp[:,j])
    imps.append(np.mean(np.abs(score(Xp) - base)))
imps = np.array(imps); imps /= imps.sum()
...
ax.fill_between(range(len(s_test)), s_test, threshold,
                where=(s_test>threshold), color=ORANGE, alpha=0.25, label="Flagged")
ax.set_xlabel("Window index"); ax.set_ylabel("Anomaly score")
